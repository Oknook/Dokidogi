...
public class MatchService {
    ...
    public List<User> findUsersWithin2km(double latitude, double longitude) {
        return userRepository.findUsersWithinDistance(latitude, longitude, 2.0);
    }
...
//
    ...
    public List<PetDTO> filterPets(MatchDTO matchDTO, List<User> userInList) {
        ...
        List<Long> filteredUserIds = userInList.stream()
            ...
            .filter(user -> matchDTO.calculateAge(matchDTO.getBirth()) == 0
                || user.calculateAge(user.getBirth()) == matchDTO.calculateAge(matchDTO.getBirth()))
            // userId만 받는 리스트로 만든다
            .map(User::getUserId)
            // 필터들을 통해 만들어진게 합쳐진다
        ...
        for (Long id : filteredUserIds) {
            User user = userRepository.findByUserId(id);
            List<Relation> pets = relationRepository.findRelationsByUser(user);
            relations.addAll(pets);
        }
